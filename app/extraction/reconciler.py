...
def _domain_of(url: str) -> str:
    if url.startswith("searxng://"):
        return url  # pseudo-source — treat each as its own independent signal
    try:
...
def reconcile(
    ...
    # Corroboration: a value confirmed by several independent domains is more
    # trustworthy than a single source's self-rated confidence; a lone source
    # is less trustworthy. Applied after the single-candidate cap so agreement
    # can legitimately raise confidence above one extractor's own estimate.
    distinct_domains = {_domain_of(c.source.url) for c in winning_group}
